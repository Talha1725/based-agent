...
const fetchWaitlistData = async () => {
	...
	console.log('waiting_list_users', waiting_list_users);
...
const WaitlistLeaderboard = () => {
	...
	const {
		data: waitlistData, isLoading, error,
	} = useQuery({
		queryKey: ["waitlistData"], queryFn: fetchWaitlistData,
	});
	
	const Error = ({message}) => (<div className="flex items-center justify-center h-screen">
	...
	const filteredData = waitlistData.filter((member) => member.email.toLowerCase().includes(searchTerm.toLowerCase()));
	
	return (<div className="min-h-screen bg-gray-100  text-gray-800 ">
			...
			<Card className="bg-white  text-gray-800  mb-8 rounded-none border-2 border-black">
				...
				<CardContent>
					...
					<Table className="w-full">
						...
						<TableBody>
							{filteredData.map((member) => (<TableRow key={member.id}>
